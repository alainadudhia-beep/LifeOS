  ...
  return (
    <div className={mobile ? 'lm-mobile-container' : undefined}>
      ...
      <div className="lm-row">
        <div className="lm-label"><span className="lm-label-emoji">🔥</span> Calories</div>
        <div className="lm-day-grid" style={{ width: gridWidth }}>
          <WeekLines days={gridDays} />
          {gridDays.map((d, i) => {
            const iso    = d.toISOString().slice(0, 10)
            ...
            return (
              <div
                ...
                style={{ left: i * DAY_WIDTH + 1, width: DAY_WIDTH - 2, background: bg || undefined }}
                title={active != null ? `Active: ${Math.round(active)} kcal  Resting: ${Math.round(resting ?? 0)} kcal` : undefined}
              >
                {total != null && (
                  <span className="lm-cell-label">{total >= 1000 ? `${(total / 1000).toFixed(1)}k` : total}</span>
                )}
              </div>
            )
          })}
      ...
      {/* ── Weight ── */}
      <div className="lm-row">
        <div className="lm-label"><span className="lm-label-emoji">⚖️</span> Weight</div>
        <div className="lm-day-grid" style={{ width: gridWidth }}>
          <WeekLines days={gridDays} />
          {gridDays.map((d, i) => {
            const iso = d.toISOString().slice(0, 10)
            ...
            return (
              <div
                key={iso}
                className={`lm-cell ${d.getDay() === 1 ? 'lm-cell--week-start' : ''}`}
                style={{ left: i * DAY_WIDTH + 1, width: DAY_WIDTH - 2, background: kg != null ? '#e0f2fe' : undefined }}
              >
                {kg != null && <span className="lm-cell-label">{kg % 1 === 0 ? kg : kg.toFixed(1)}</span>}
              </div>
            )
          })}
        </div>
        {mobile && (() => {
          ...
          return (
            <div className="lm-today-col">
              <div
                className="lm-cell"
                style={{ left: 1, width: DAY_WIDTH - 2, background: kg != null ? '#e0f2fe' : undefined }}
              >
                {kg != null && <span className="lm-cell-label">{kg % 1 === 0 ? kg : kg.toFixed(1)}</span>}
              </div>
            </div>
